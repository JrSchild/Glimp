...
- (void)initControlBtn {
    UIButton *retakeBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 50.0f, 100, 50)];
    retakeBtn.backgroundColor = [UIColor blackColor];
    retakeBtn.titleLabel.textColor = [UIColor whiteColor];
    [retakeBtn setTitle:@"Retake" forState:UIControlStateNormal];
    [retakeBtn.titleLabel setFont:[UIFont boldSystemFontOfSize:18.0f]];
    [retakeBtn.titleLabel setTextAlignment:NSTextAlignmentCenter];
    [retakeBtn.titleLabel setLineBreakMode:NSLineBreakByWordWrapping];
    [retakeBtn.titleLabel setNumberOfLines:0];
    [retakeBtn setTitleEdgeInsets:UIEdgeInsetsMake(5.0f, 5.0f, 5.0f, 5.0f)];
    ...
    UIButton *cancelBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.view.frame.size.width - 100.0f, 0, 100, 50)];
...
- (void)retake:(id)sender {
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(VPImageCropperDelegate)]) {
        [self.delegate imageCropperShouldRetake:self];
    }
}
